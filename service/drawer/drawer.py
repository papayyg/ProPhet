...
OPEN_SANS = ImageFont.truetype('service/drawer/OpenSans.ttf', FONT_SIZE)
bold_font = ImageFont.truetype("service/drawer/OpenSansBold.ttf", FONT_SIZE)
...
class BubbleDrawer:
    ...
    def _name(self) -> str:
        try:
            result = self.message["from"]["first_name"]
            if self.message["from"]["last_name"]:
                result += " " + self.message["from"]["last_name"]
            return result
    ...
    def _text(self) -> List[str]:
        return textwrap.wrap(self.message["text"], width=WIDTH_SIZE)

    def set_avatar(self, message) -> None:
        # Open original image
        avatar = Image.open(f"{message}")  # type: Image.Image
    ...
    def draw(self) -> None:
        ...
        d.text((TEXT_X_START, BUBBLE_PADDING), self._name, fill="pink", font=bold_font)
